...
const searchAPK = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
    if (!text) return m.reply('Yo, gimme somethin\' to search for.');
    ...
    try {
      ...
      if (topAPKs.length === 0) {
        m.reply('Aint no APKs found, bro.');
        await m.React("❌");
        return;
      }
          ...
          message: {
            ...
            interactiveMessage: proto.Message.InteractiveMessage.create({
              body: proto.Message.InteractiveMessage.Body.create({
                text: `America-Md 🇺🇲 APK Downloader\n\n🔍 Search and snag your favorite APKs, homie.\n\n📌 Just pick one from the list below and let's roll.\n\n`
              }),
      ...
      m.reply('Yo, there was an issue handling your request.');
      await m.React("❌");
    }
  } else if (selectedId) { 
    ...
    if (selectedAPK) {
      try {
        ...
        await Matrix.sendMessage(m.from, { image: { url: iconUrl }, caption: `You picked this APK:\n\nName: ${selectedAPK.name}\nSize: ${size}\n\n> © Powered by America-Md 🇺🇲` }, { quoted: m });
        ...
        m.reply('There was an issue sending the APK.');
      }
    }
  }
};
