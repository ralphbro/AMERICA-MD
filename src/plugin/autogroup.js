...
const groupSetting = async (m, gss) => {
  try {
    ...
    if (!m.isGroup) return m.reply("*📛 YO, THIS COMMAND IS ONLY FOR GROUPS!*");
    ...
    if (!botAdmin) return m.reply("*📛 BOT'S GOTTA BE AN ADMIN FOR THIS!*");
    if (!senderAdmin) return m.reply("*📛 YOU NEED TO BE AN ADMIN TO USE THIS!*");

    const args = m.body.slice(prefix.length + cmd.length).trim().split(/\s+/);
    if (args.length < 1) return m.reply(`Yo, specify a setting (open/close) and maybe a time.\n\nExample:\n*${prefix + cmd} open* or *${prefix + cmd} open 04:00 PM*`);
    ...
    scheduledTasks[m.from] = cron.schedule(cronTime, async () => {
      try {
        ...
        if (groupSetting === 'close') {
          await gss.groupSettingUpdate(m.from, 'announcement');
          await gss.sendMessage(m.from, { text: "Aight, the group's closed." });
        } else if (groupSetting === 'open') {
          await gss.groupSettingUpdate(m.from, 'not_announcement');
          await gss.sendMessage(m.from, { text: "Aight, the group's open." });
        }
      } catch (err) {
    ...
    m.reply(`Group's gonna be set to "${groupSetting}" at ${time} IST.`);
  } catch (error) {
    console.error('Error:', error);
    m.reply('Yo, there was an error processing the command.');
  }
};
